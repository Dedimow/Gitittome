...
sourcecsv = Path(r"E:\Python Scripts\UtilityPackage\UtilSourceDict.csv")

sourcedirectorypaths = {}

with sourcecsv.open("r",newline="",encoding="utf-8-sig") as sourceread:
    sources = csv.DictReader(sourceread)
    
    for row in sources:
        sourcedirectorypaths[row['ButtonName']] = row['DirectoryPath']
        
#create a dictionary for all destination directory paths ----- Want to set this up as a separate callable CSV
destcsv = Path(r"E:\Python Scripts\UtilityPackage\UtilDestDict.csv")

destdirectorypaths = {}

with destcsv.open("r",newline="",encoding="utf-8-sig") as destread:
    dests = csv.DictReader(destread)

    for row in dests:
        destdirectorypaths[row['Button_Name']] = row['Directory_Path']
...
#create button for OBS source folder
OBSBtn = tk.Button  (root, 
                    text = "OBS Captures",
                    width = 13,
                    bg = 'blue', 
                    fg = "white",
                    command=lambda : selectsource("OBS Captures"))

OBSBtn.place(x=145,y=85)
...
#create button for OBS destination folder
OBSDestBtn = tk.Button  (root, 
                    text = "OBS Backups",
                    width = 13,
                    bg = 'green',
                    command =lambda: selectdest("OBS Backups"))

OBSDestBtn.place(x=768,y=85)

#create button for Tester destination folder
